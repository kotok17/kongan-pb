...
class KegiatanController extends BaseController
{
  ...
  public function simpan()
  {
    ...
    $tanggalKegiatan = $this->request->getPost('tanggal_kegiatan');

    if ($this->kegiatanModel->hasKegiatanWithinYears((int) $idAnggota, $tanggalKegiatan)) {
      return redirect()->back()
        ->withInput()
        ->with('error', 'Anggota ini sudah memiliki kegiatan lain dalam rentang 3 tahun. Pilih tanggal lain.');
    }
  ...
  {
    $kegiatan = $this->kegiatanModel->getDetailWithOwner($id);
    if (!$kegiatan) {
      return redirect()->to('/kegiatan')->with('error', 'Kegiatan tidak ditemukan');
    }

    $role       = session()->get('role');
    $userId     = session()->get('id_anggota');
    $isOwner    = (int)($kegiatan['id_anggota'] ?? 0) === (int)$userId;
    ...
    $konganModel = new \App\Models\DetailKegiatanModel();
    $kongan      = $konganModel->getKonganByKegiatan($id);

    $anggota = [];
    if ($canManage) {
      $anggotaModel = new \App\Models\AnggotaModel();
      $anggota      = $anggotaModel->orderBy('nama_anggota')->findAll();
    }
  ...
  {
    $idKegiatan = (int)$this->request->getPost('id_kegiatan');
    ...
    $file = $this->request->getFile('import_file'); // <— gunakan nama input yang benar
    ...
    try {
      ...
      for ($i = 2; $i <= count($rows); $i++) { // mulai dari baris kedua
        $row = $rows[$i];
        $namaAnggota = trim($row['A'] ?? '');
        $jumlah      = (int)preg_replace('/\D/', '', (string)($row['B'] ?? ''));

        if ($namaAnggota === '' || $jumlah <= 0) {
          $errors[] = "Baris $i: Data tidak lengkap atau jumlah tidak valid";
          continue;
        }

        $anggota = $this->anggotaModel->where('nama_anggota', $namaAnggota)->first();
        if (!$anggota) {
          $errors[] = "Baris $i: Anggota '$namaAnggota' tidak ditemukan";
          continue;
        }

        $exists = $db->table('kegiatan_detail')
          ->where('id_kegiatan', $idKegiatan)
          ->where('id_anggota', $anggota['id_anggota'])
          ->get()->getRowArray();

        if ($exists) {
          $errors[] = "Baris $i: Anggota '$namaAnggota' sudah memiliki data kongan";
          continue;
        }
